...
Page({
  ...
  init() {
    ...
    this.setData({ prize: _prize, curNumDeg: _curNumDeg })
  },
  //点击开始抽奖
  start() {
    if (this.data.isPrize) {
      ...
      let _needleDeg = (this.data.prize_cur[_num].num + surplus) * this.data.curNumDeg + this.data.needleDeg + (360 - this.data.needleDeg % 360) + 1800
